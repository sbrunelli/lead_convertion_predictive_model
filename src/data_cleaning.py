    ...
    def _create_customer_metrics(self):
        ...
        * Amount CHF last loss
        ...
        def first_contact():
            ...
            self.data['Customer.First.Contact'] = self.data['Customer.Contacts.So.Far'].map(lambda x: 1 if x==1 else 0)
            self.data.set_index('Opportunity.Number')
        ...
        def conv_rate_so_far():
            ...
            self.data['Customer.ConvRatio.So.Far'] = self.data['Customer.Won.So.Far'].astype('float') / (self.data['Customer.Contacts.So.Far']-1)
        ...
        def conv_rate_last_five():
            ...
            print cvrl5[cvrl5.Customer == cust].head(10)
            assert False
            ...
            # Shift it by 1 to avoid leakage
            self.data['Customer.ConvRatio.Last5'] = self.data.groupby('Customer')['Customer.ConvRatio.Last5'].shift(1)
        ...
        def conv_rate_last_year():
            ...
            # Shift it by 1 to avoid leakage
            self.data['Customer.ConvRatio.LastYear'] = self.data.groupby('Customer')['Customer.ConvRatio.LastYear'].shift(1)
